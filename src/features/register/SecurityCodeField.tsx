...
const SecurityCodeField = () => {
  const { value, onChange } = useInput({ defaultValue: '', sanitize: onlyNumeric });
  ...
  return (
    <Box className='field-container' style={{ width: '50%' }}>
      ...
      <Input className='field-input' type='password' value={value} onChange={onChange} maxLength={MAX_LENGTH} />
    </Box>
  );
};
